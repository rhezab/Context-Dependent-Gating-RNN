...
class Model:
	...
	def __init__(self, input_data, target_data, mask, gating, lin_dropout, lesion_mask):
	...
	def declare_variables(self):
		...
		base_var_prefies    = ['W_out', 'b_out', 'W_lin', 'b_lin', 'W_rec', 'b_rec']
	...
	def pathint_stabilization(self, adam_optimizer):
		...
		self.gradients = optimizer_task.compute_gradients(self.pol_loss + self.rec_loss)
...
def supervised_learning(save_fn='test.pkl', gpu_id=None):
	...
	d = tf.placeholder_with_default(0.5, [], 'lin_dropout')
	l = tf.placeholder_with_default(np.ones([par['batch_size'],par['n_linear']]).astype(np.float32), [par['batch_size'],par['n_linear']], 'lesion')
		...
		for task in range(par['n_tasks']):
			for i in range(par['n_train_batches']):
				...
				if i%500 == 0:
					...
					print('Iter: {:>4} | Task name: {} | Acc: {:5.3f} | Task Loss: {:5.3f} | Recon Loss: {:5.3f} | '
						'Aux Loss: {:5.3f} | Spike Loss: {:5.3f}'.format(i, name, acc, loss, recon_loss, AL, spike_loss))
