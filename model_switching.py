...
class Model:
    ...
    def declare_variables(self):
        ...
        # Lists of prefixes
        lstm_solver_var_prefixes = ['s_Wf', 's_Wi', 's_Wo', 's_Wc', 's_Uf', 's_Ui', 's_Uo', 's_Uc', 's_bf', 's_bi', 's_bo', 's_bc']
        lstm_gater_var_prefixes = ['g_Wf', 'g_Wi', 'g_Wo', 'g_Wc', 'g_Uf', 'g_Ui', 'g_Uo', 'g_Uc', 'g_bf', 'g_bi', 'g_bo', 'g_bc']
        ...
        prefix_list += lstm_solver_var_prefixes
        prefix_list += lstm_masker_prefixes
    ...
    def rnn_cell_loop(self):
        ...
        h_gater = tf.zeros_like(par['h_init'])
        c_gater = tf.zeros_like(par['h_init'])
        h_solver = tf.zeros_like(par['h_init'])
        c_solver = tf.zeros_like(par['h_init'])
        mask  = self.mask[0]
        ...
        for rnn_input, target, time_mask in zip(self.input_data, self.target_data, self.time_mask):
            ...
            h_gater, c_gater = self.recurrent_cell(h_gater, c_gater, rnn_input) # gater needs to fed most recent (x,a,r)
            h_solver, c_solver = self.recurrent_cell(h_solver, c_solver, rnn_input)

            # Record hidden state
            self.h_gater.append(h_gater)
            self.h_solver.append(h_solver)
    ...
    def recurrent_cell(self, h, c, rnn_input):
